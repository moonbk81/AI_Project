...
1. 결함을 선택하면 첨부 목록을 조회하고, **ZIP 첨부를 자동으로 내려받습니다**.
2. ZIP 안(중첩 ZIP 포함)에서 **로그 파일 이름 패턴에 맞는 파일만** 추출합니다.
...
| 모듈 | 역할 |
|---|---|
| `core/log_archive.py` | ZIP 열기/목록/추출, 로그 파일 이름 패턴. Streamlit·파일시스템·네트워크 의존 없음 |
...
- **중첩 ZIP 최대 깊이 3** (`NESTED_ZIP_MAX_DEPTH`) — PLM 첨부는 ZIP 안에 ZIP 이
  다시 들어있는 경우가 많고, 로그는 보통 안쪽에 있습니다.
...
화면에 ZIP 안의 실제 파일 이름이 함께 표시됩니다. 그 이름이 위 패턴(dumpstate 계열)이
아니면 자동 인식 대상이 아닙니다. "검색 및 파일" 탭에서 **📂 Open** 으로 ZIP 내용을
펼친 뒤 원하는 파일을 직접 선택해 분석에 넣을 수 있습니다.

**목록이 비어 보임**
`📂 Open` 이 쓰는 목록은 ZIP **최상위** 파일만 보여줍니다. 하위 폴더나 중첩 ZIP
...
from core.log_archive import (
    extract_logs_from_zip,   # {파일명: bytes} — 중첩 ZIP 까지 훑어 로그만
    extract_file,            # ZIP 에서 이름 하나 꺼내기 (폴더 안에 있어도 찾음)
    list_zip_contents,       # 최상위 {파일명: 크기}
    list_archive_contents,   # 중첩 포함 {표시경로: 크기}
    is_log_file,             # 이름이 로그 패턴과 맞는지
)

from plm.log_pipeline import (
    select_zip_attachments,        # 첨부 중 ZIP 만

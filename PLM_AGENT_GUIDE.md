...
- 에이전트도 웹 화면과 같은 후보 스캔을 먼저 실행합니다. 추천 점수가 가장 높은
  로그 하나만 분석하며, 같은 점수의 후보가 여럿이면 동점인 후보를 모두 분석합니다.
  추천 점수는 파일 종류별로 같으므로 동점은 흔합니다. 동점 후보 중 크기가 같은
  파일은 같은 로그의 사본으로 보고 가장 바깥 경로 하나만 남깁니다. 검색 근거를
  얻지 못하면 전체 후보로 한 번 확대하여 재분석합니다.
- 전체 덤프(`dumpstate.txt`, `act_dumpstate.txt`)만 최상위 등급이며,
  `dumpstate_board.txt`, `dumpstate_log.txt`, `dumpState_FOTA_PROVIDER_*.log`처럼
  이름만 비슷한 파일은 아래 등급으로 내려갑니다.
